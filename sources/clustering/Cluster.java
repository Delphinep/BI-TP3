...
public class Cluster implements java.lang.Iterable<Donnee>{
    ...
    public void add(Donnee d) throws ClusterException{
        ...
        for(int i=0;i<d.nbDimensions();i++){
            ...
            this.somme[i] += vali ;
        	if((d.valeurDim(i)<this.min[i])||this.premiereDonnee){
        		this.min[i] = d.valeurDim(i);
        	}
        	if(d.valeurDim(i)>this.max[i]){
        		this.max[i] = d.valeurDim(i);
        	}
   ...
   /* public Donnee ecartType(){
    ...
    public double wc() throws ClusterException{
        ...
        for(int i=0;i<this.data.size();i++){
        	som += this.data.get(i).distanceCentre();
        }
        return som ;
    }
