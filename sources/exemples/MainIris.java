...
public class MainIris {
			...
			do {
				...
				if (ligne != null && ligne.length() > 0) {
					...
					if (ligne.charAt(0) != '@' && ligne.charAt(0) != '%') {
						...
						data.add(new DonneeIris(array(
								Double.parseDouble(ligneCoupee[0]),
	...
	private static void ecrireFichier(Cluster cl, String fichier) {
		...
		try {
			BufferedWriter ficTexte = new BufferedWriter(new FileWriter(
					cheminFichier, false));
			for (Donnee d : cl) {
				ficTexte.write(d.valeurDim(0) + " " + d.valeurDim(1) + " "
						+ d.valeurDim(2) + " " + d.valeurDim(3) + "\n");
			}
			ficTexte.flush();
			ficTexte.close();
		} catch (FileNotFoundException e) {
	...
	private static void afficheDetail(int i, Cluster cl) {
		...
		int nbVersicolor = 0;
		int nbVirginica = 0;
		System.out.println("le cluster " + i + " a " + cl.size()
				+ " donnees et son centre est " + cl.moyenne());
		for (Donnee d : cl) {
			if (((DonneeIris) d).nom().equals("Iris-setosa"))
				nbSetosa++;
			else if (((DonneeIris) d).nom().equals("Iris-versicolor"))
				nbVersicolor++;
			else
				nbVirginica++;
		}
		System.out.println("--> Détail : " + nbSetosa + " iris setosa, "
				+ nbVersicolor + " iris versicolor, " + nbVirginica
				+ " iris virginica");
	}
	...
	public static void main(String argv[]) throws ClusterException {
		 ...
		 * "il faut deux parametres, le fichier et le nombre de clusters.");
		 * System.out.println(
		 * "Par exemple : java -cp ./classes exemples.MainIris iris.arff 3") ;
		 * }else {
		 */
		// Cluster lesDonnees = lireFichier(argv[0]) ;
		Cluster lesDonnees = lireFichier("doc/resources/iris.arff");
